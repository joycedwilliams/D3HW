...
function xScale(census_data, chosenXAxis) {
  ...
  var xLinearScale = d3.scaleLinear()
    .domain([d3.min(census_data, d => d[chosenXAxis]) * .75,
      d3.max(census_data, d => d[chosenXAxis]) * 2.5
    ])
    .range([0, width]);
...
d3.csv("census_data.csv", function(err, census_data) {
  if (err) throw err;
  ...
  var yLinearScale = d3.scaleLinear()
    .domain([0, d3.max(census_data, d => d.obesity)])
    .range([height, 0]);
